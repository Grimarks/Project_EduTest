...
const categories = ["All", "Mathematics", "English", "Science"];
        ...
        const fetchClasses = async () => {
            ...
            try {
                ...
                const formattedClasses = response.data.map(cls => ({
                    ...cls,
                    id: String(cls.id),
                    features: cls.features || ["Feature 1", "Feature 2", "Feature 3"]
                }));
                        ...
                        filteredClasses.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {filteredClasses.map((premiumClass, index) => (
                                    <Card key={premiumClass.id} className="group hover:shadow-hover transition-smooth animate-slide-up bg-gradient-card border-border/50" style={{ animationDelay: `${index * 0.1}s` }}>
                                        <CardHeader className="space-y-3">
                                            ...
                                            <div className="flex items-start justify-between">
                                                <Badge variant="outline" className="text-xs">{premiumClass.category}</Badge>
                                                <Badge className="bg-accent text-accent-foreground">Premium</Badge>
                                            </div>
                                            <CardTitle className="text-lg group-hover:text-primary transition-smooth">{premiumClass.title}</CardTitle>
                                            <p className="text-sm text-muted-foreground line-clamp-2">{premiumClass.description}</p>
                                        </CardHeader>
                                        ...
                                        <CardContent className="space-y-4">
                                            ...
                                            <div className="flex items-center justify-between pt-2 border-t border-border">
                                                ...
                                                <span className="text-lg font-bold text-accent">{formatPrice(premiumClass.price)}</span>
                                            </div>
                                        </CardContent>
                            ...
                            <p className="text-center text-muted-foreground">No classes found for this category.</p>
                        )
                    )}
                </div>
            </section>
